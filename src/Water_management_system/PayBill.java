...
public class PayBill extends JFrame implements ActionListener{
    ...
    PayBill(String meter) {
        ...
        JLabel heading = new JLabel("Water Bill");
        ...
        try {
            ...
            while(rs.next()) {
                ...
                labelname.setText(rs.getString("tfname"));
            }
            
            rs = c.s.executeQuery("select * from bill where meternumber = '"+meter+"' AND cmonth = 'January'");
            while(rs.next()) {
                labelunits.setText(rs.getString("tfunits"));
        ...
        cmonth.addItemListener(new ItemListener(){
            ...
            public void itemStateChanged(ItemEvent ae) {
                try {
                    Conn c = new Conn();
                    ResultSet rs = c.s.executeQuery("select * from bill where meternumber = '"+meter+"' AND cmonth = '"+cmonth.getSelectedItem()+"'");
                    while(rs.next()) {
                        labelunits.setText(rs.getString("tfunits"));
                        labeltotalbill.setText(rs.getString("totalbill"));
                        labelstatus.setText(rs.getString("status"));
    ...
    public void actionPerformed(ActionEvent ae) {
        if (ae.getSource() == pay) {
            try {
                Conn c = new Conn();
                c.s.executeUpdate("update bill set status = 'Paid' where meterumber = '"+meter+"' AND cmonth='"+cmonth.getSelectedItem()+"'");
            } 
            catch (Exception e) {
                e.printStackTrace();
            }
