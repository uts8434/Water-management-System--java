...
import Water_management_system.Conn;
import javax.swing.*;
import java.awt.*;
import java.util.*;
...
public class CalculateBill extends JFrame implements ActionListener{
    ...
    CalculateBill() {
        ...
        try {
            ...
            while(rs.next()) {
                meternumber.add(rs.getString("lblmeter"));
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
        ...
        try {
            Conn c = new Conn();
            ResultSet rs = c.s.executeQuery("select * from customer where lblmeter = '"+meternumber.getSelectedItem()+"'");
            while(rs.next()) {
                lblname.setText(rs.getString("tfname"));
                labeladdress.setText(rs.getString("tfaddress"));
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public void actionPerformed(ActionEvent ae) {
        if (ae.getSource() == next) {
            ...
            String query2 = "insert into bill values('"+meter+"','"+units+"', '"+month+"',  'Not Paid', '"+totalbill+"')";
